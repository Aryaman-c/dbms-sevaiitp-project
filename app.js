...
// Helper function to clean up expired campaigns (used by both endpoint and scheduler)
async function cleanupExpiredCampaigns() {
    try {
        ...
        try {
            ...
            const [expiredCamps] = await connection.execute(
                'SELECT id FROM blood_donation_camps WHERE date < ?', [localDate]
            );
            if (expiredCamps.length > 0) {
                const campIds = expiredCamps.map(c => c.id);
                const placeholders = campIds.map(() => '?').join(',');
                await connection.execute(
                    `DELETE FROM blood_donation_camps WHERE id IN (${placeholders})`, campIds
                );
            }

            // Fundraising campaigns
            const [expiredFundraising] = await connection.execute(
                'SELECT id FROM fundraising_campaigns WHERE end_date < ?', [localDate]
            );
            if (expiredFundraising.length > 0) {
                const fundraisingIds = expiredFundraising.map(c => c.id);
                const placeholders = fundraisingIds.map(() => '?').join(',');
                await connection.execute(
                    `DELETE FROM fundraising_donations WHERE campaign_id IN (${placeholders})`, fundraisingIds
                );
                await connection.execute(
                    `DELETE FROM fundraising_campaigns WHERE id IN (${placeholders})`, fundraisingIds
                );
            }

            // Item donation campaigns
            const [expiredItemCampaigns] = await connection.execute(
                'SELECT id FROM item_donation_campaigns WHERE end_date < ?', [localDate]
            );
            if (expiredItemCampaigns.length > 0) {
                const itemIds = expiredItemCampaigns.map(c => c.id);
                const placeholders = itemIds.map(() => '?').join(',');
                await connection.execute(
                    `DELETE FROM item_donations WHERE campaign_id IN (${placeholders})`, itemIds
                );
                await connection.execute(
                    `DELETE FROM item_donation_campaigns WHERE id IN (${placeholders})`, itemIds
                );
            }

            await connection.commit();
            connection.release();
            console.log('Expired campaigns cleaned up automatically.');
        } catch (err) {
            await connection.rollback();
            connection.release();
            console.error('Error during expired campaign cleanup (auto):', err);
        }
    } catch (err) {
        console.error('Error cleaning up expired campaigns (auto):', err);
    }
}

// Schedule automatic cleanup every minute
function scheduleMinuteCleanup() {
    setInterval(cleanupExpiredCampaigns, 60 * 1000); // Every 1 minute
}
scheduleMinuteCleanup();
